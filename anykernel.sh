...
# begin properties
properties() { '
kernel.string=Rave kernel by Peppe289 @ telegram
do.devicecheck=1
do.modules=1
do.cleanup=1
do.cleanuponabort=0
...
if [ -z $NEW ]; then
  ...
  ui_print "   Choose the type rom:"
  ui_print "   Volume Up = HMP, Volume Down = EAS"
  if $FUNCTION; then
    NEW=true
  else
    NEW=false
...
if [ -f $compressed_image ]; then
  ...
  if $NEW; then #true
    cat $compressed_image /tmp/anykernel/dtbs/*.dtb-HMP > /tmp/anykernel/Image.gz-dtb;
    ui_print "   HMP rom"
  else #false, con EAS energy cost
    cat $compressed_image /tmp/anykernel/dtbs/*.dtb-EAS > /tmp/anykernel/Image.gz-dtb;
    ui_print "   EAS rom"
  fi
fi;
